...
def _symlink_or_copy(dest: Path, target: Path, *, relative_target: str | None = None) -> None:
    """Create a run-local view of a shared file.

    - On Windows: avoid symlinks (copy instead) since symlinks often require
      admin/Developer Mode.
    - Elsewhere: prefer symlink, fallback to copy if symlink fails.
    """
    dest.parent.mkdir(parents=True, exist_ok=True)

    # If a broken symlink exists, Path.exists() is False but the entry still
    # blocks creation.
    if dest.is_symlink():
        try:
            dest.unlink()
        except OSError:
            # If we can't remove it, bail out rather than crashing the run.
            return
    ...
    try:
        dest.symlink_to(relative_target if relative_target is not None else target)
    except OSError:
        shutil.copy2(target, dest)
            ...
            if fulltext_file.exists():
                ...
                if run_id:
                    ...
                    if not run_fulltext_symlink.exists():
                        _symlink_or_copy(
                            run_fulltext_symlink,
                            fulltext_file,
                            relative_target=f"../../shared/{pmc_id}.fulltext.html",
                        )
                        logger.debug(f"Created run copy/link for {pmc_id} in run {run_id}")
            ...
            if metadata_file.exists():
                ...
                if run_dir:
                    ...
                    if not run_metadata_symlink.exists():
                        _symlink_or_copy(
                            run_metadata_symlink,
                            metadata_file,
                            relative_target=f"../../shared/{paper_id}.metadata.json",
                        )
            ...
            if papers_to_supplement:
                ...
                for paper_id, metadata in papers_to_supplement:
                    ...
                    if not run_metadata_symlink.exists():
                        _symlink_or_copy(
                            run_metadata_symlink,
                            shared_dir / f"{paper_id}.metadata.json",
                            relative_target=f"../../shared/{paper_id}.metadata.json",
                        )
                    ...
                    if not run_fulltext_symlink.exists():
                        _symlink_or_copy(
                            run_fulltext_symlink,
                            shared_dir / f"{pmc_id}.fulltext.html",
                            relative_target=f"../../shared/{pmc_id}.fulltext.html",
                        )
