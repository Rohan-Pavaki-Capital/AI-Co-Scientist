        ...
        if pmc_id:
            try:
                ...
                if html_file.exists():
                    ...
                    if text and not text.startswith(EXTRACTION_ERROR_PREFIX):
                        papers_with_fulltext += 1
                        logger.debug(f"extracted {len(text)} chars from {pmc_id}")
                    else:
                        logger.warning(f"Fulltext extraction returned error marker for {pmc_id}")
                else:
