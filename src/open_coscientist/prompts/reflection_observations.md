...
```json
{
  "hypothesis_text": "<copy the hypothesis text exactly>",
  "reasoning": "<full reasoning for steps 1-4>",
  "classification": "<already explained|other explanations more likely|missing piece|neutral|disproved>"
}
```

Rules:
- Return raw JSON only (no markdown, no prose before/after JSON).
- Use one classification value exactly from the allowed list.
- Keep all analysis content inside `reasoning`.
