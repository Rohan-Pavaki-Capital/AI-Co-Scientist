...
from .llm import describe_model_backend
...
def _compute_recursion_limit(max_iterations: int) -> int:
    """
    Compute a safe LangGraph recursion limit from configured iterations.

    Baseline flow uses ~6 node steps before iteration loops. Each additional
    refinement iteration can add up to ~5 node steps (meta_review, evolve,
    review, ranking, proximity). We add generous buffer for conditional paths.
    """
    return max(100, 40 + (max_iterations * 10))
...
class HypothesisGenerator:
    ...
    def _build_graph(self, enable_literature_review_node: bool = True) -> StateGraph:
        ...
        def after_ranking(state: WorkflowState) -> str:
            ...
            # Tournament requires at least 2 hypotheses. If we drop below that,
            # we cannot make meaningful pairwise progress, so stop gracefully.
            if hypothesis_count < 2:
                logger.info(
                    "Ending workflow: only %d hypothesis available after ranking",
                    hypothesis_count,
                )
                return "end"
    ...
    async def _prepare_generation(
        ...
        model_info = describe_model_backend(self.model_name)
        ...
        logger.info(
            "LLM runtime config | engine=%s provider=%s model=%s api_key_env=%s api_key_present=%s",
            model_info.get("engine"),
            model_info.get("provider"),
            self.model_name,
            api_key_env or "unknown",
            api_key_present,
        )
    ...
    async def _generate_hypotheses_without_streaming(
        ...
        try:
            # Run workflow with a dynamic recursion limit derived from max_iterations.
            recursion_limit = _compute_recursion_limit(initial_state.get("max_iterations", 0))
