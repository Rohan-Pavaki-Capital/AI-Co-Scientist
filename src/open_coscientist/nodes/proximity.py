...
async def proximity_node(state: WorkflowState) -> Dict[str, Any]:
    ...
    def _skip_and_advance(reason: str) -> Dict[str, Any]:
        """Skip deduplication safely while still advancing iteration state."""
        current_iteration = state.get("current_iteration", 0)
        next_iteration = current_iteration + 1
        logger.info(
            "Skipping proximity analysis (%s); advancing iteration to %d",
            reason,
            next_iteration,
        )
        return {
            ...
            "messages": [
                {
                    "role": "assistant",
                    "content": f"Skipped deduplication: {reason}",
                    "metadata": {
                        "phase": "proximity",
                        "skipped": True,
                        "reason": reason,
    ...
    if len(hypotheses) <= 1:
        ...
        return _skip_and_advance("not_enough_hypotheses")
    ...
    if not similarity_clusters:
        similarity_clusters = (
            ...
            or response.get("clustered_hypotheses")
            or response.get("groups")
            or []
        )

    if not isinstance(similarity_clusters, list):
        logger.warning("Unexpected proximity output format, skipping deduplication")
        return _skip_and_advance("unexpected_output_format")

    if not similarity_clusters:
        logger.warning("No similarity clusters returned, skipping deduplication")
        return _skip_and_advance("no_similarity_clusters")
