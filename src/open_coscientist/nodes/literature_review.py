...
async def _phase4_synthesize(
    ...
    try:
        ...
        logger.error(f"Synthesis exception traceback:\n{traceback.format_exc()}")
        return LITERATURE_REVIEW_FAILED
...
async def literature_review_node(state: WorkflowState) -> Dict[str, Any]:
    ...
    if cached is not None:
        cached_synthesis = cached.get("articles_with_reasoning")
        if cached_synthesis == LITERATURE_REVIEW_FAILED:
            logger.warning(
                "Ignoring cached literature review result with failed synthesis marker; recomputing"
            )
        else:
    ...
    logger.info(f"Phase 3 complete: {len(paper_analyses) if paper_analyses else 0} papers analyzed")

    # phase 4: synthesize
    if paper_analyses:
        logger.info(f"Phase 4: calling synthesis with {len(paper_analyses)} paper analyses")
        synthesis = await _phase4_synthesize(paper_analyses, state, background_context)
        logger.info(f"Phase 4 complete: synthesis is {len(synthesis) if synthesis != LITERATURE_REVIEW_FAILED else 'FAILED'} chars")
    else:
        logger.error("Phase 3 returned no paper analyses")
    ...
    if synthesis == LITERATURE_REVIEW_FAILED:
        logger.error("Literature review synthesis failed - returning failure result")
        await emit_progress(
            state,
            "literature_review_error",
            "Literature review failed (synthesis error)",
            0.2,
        )
        result = make_failure_result("synthesis failed", queries=queries, articles=articles)
